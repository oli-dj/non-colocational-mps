...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Addded for Multilayer Peceptron training:
save_d = 0;
save_polar =0;

if save_d == 1
    fid = fopen('nn_data_polar3new.txt','a');
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
switch dim % Switch for 2D and 3D. TODO: Implement 1D support too.
    case 2 % 2D
        ...
        for i = 1:n_u
            ...
            num_informed = 0;
            if save_d == 1
                d_list_entry =  NaN(cap,3);
                if save_polar
                    d_list_entry =  NaN(cap,5);
                end
            end
                ...
                if num_informed < cap
                    try
                        ...
                        if ~isnan(d(h))
                            ...
                            if save_d == 1
                                %x
                                d_list_entry(num_informed,1) = tau(h,1);
                                %y
                                d_list_entry(num_informed,2) = tau(h,2);
                                if save_polar
                                    %radius
                                    d_list_entry(num_informed,3) =...
                                        sqrt(tau(h,1)^2 + tau(h,2)^2);
                                    %angle (allowed since 
                                    d_list_entry(num_informed,4) =...
                                        cart2pol(tau(h,1),tau(h,2));
                                        
                                    %outcome
                                    d_list_entry(num_informed,5) = d(h);
                                else
                                    %outcome (label)
                                    d_list_entry(num_informed,3) = d(h);
                                end
                            end
                        end
                    catch
            ...
            if save_d == 1
                d_list_entry = reshape(d_list_entry,1,[]);
            end
            ...
            if ((~isempty(find(~isnan(d),1))) || (nsd > 0))
                    ...
                    for l=1:num_combinations
                        ...
                        if prob_factor < prob_factor_limit
                            ...
                            if temps_counts_sum < threshold
                                ...
                                % Normalize and multiply with soft data
                                %probability factor
                ...
                if save_d && (sum(isnan(d_list_entry)) < 0.5)
%                     if save_polar
%                         d_list
%                         fprintf(fid, '%g, ', [d_list_entry_polar(1:end-1)]);
%                         fprintf(fid, '%g',   d_list_entry_polar(end));
%                     end
                    fprintf(fid, '%g, ', [d_list_entry probs(1)]);
                    fprintf(fid, '%g',   probs(end));
                    fprintf(fid, '\n');
                    if mod(i,1000)==0
                        fprintf('\n%i',i)
                    end
                ...
                %for k = 1:length(cat)
                %    EG(path(i,1),path(i,2),k) = probs(k);
                %end
